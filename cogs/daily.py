...
class Daily(commands.Cog):
    ...
    def trigger_action(self):
        ...
        # Daily is an additional Boss-cycle hook. Boss still owns the fixed
        # 14:00 Asia/Ho_Chi_Minh boundary and keeps its own safety timer, so a
        # delayed/missed daily command cannot break Boss ticket resets.
        boss = self.bot.get_cog("Boss")
        if boss and hasattr(boss, "on_daily_trigger"):
            asyncio.create_task(boss.on_daily_trigger())
    ...
    async def register_actions(self):
        ...
        if cfg.get('enabled', False):
            ...
            delay = 3 if remaining <= 0 else remaining
            await self.bot.neura_register_command(
                "daily", "daily", priority=self.bot.get_cmd_priority("daily", 4),
                delay=max(10, delay), initial_offset=0
            )
    ...
    async def _process_response(self, message):
        ...
        if "wait" in full_content:
            ...
            if total_seconds > 0:
                ...
                is_for_daily = "daily" in full_content or (time_since_last < 20.0)
                if is_for_daily:
